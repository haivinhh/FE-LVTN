...
const TopCustomersChart = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axiosAdmin.post('/api/getTopCustomers', {
                year: selectedYear,
                month: selectedMonth
            });
            ...
            if (Array.isArray(data) && data.length > 0) {
                const labels = data.map(item => 
                    `${item.idUser || 'Unknown ID'} - ${item.userName || 'Unknown User'} - ${item.hoTen || 'Unknown Name'}`
                );
                const amounts = data.map(item => item.tongTienDaMua || 0);
    ...
    return (
        <div style={{ width: '100%', height: '100%' }}>
            <h2>Thống Kê Khách Hàng Mua Nhiều Nhất</h2>
